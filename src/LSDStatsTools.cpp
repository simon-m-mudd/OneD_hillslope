...
// bin_data
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
// This is like the standard binning but it bins an index vector. 
// Useful for, for example, binning and index like the node index vector into 
// value bins. 
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
void bin_data(vector<int>& vector1, vector<float>& vector2, float min, float max, float bin_width, vector<float>& mid_points, vector< vector<int> >& binned_data)
{
  ...
  vector<float> mid_points_temp;
  vector< vector<int> > binned_data_temp(NBins,empty_vector);

  cout << "the size of the vectors is: " << endl;
  cout << "v1: " << vector1.size() << endl;
  cout << "v2: " << vector2.size() << endl;
...
void order_unstructured_points(vector<float> eastings, vector<float> northings, 
                               vector<float>& ordered_eastings, vector<float>& ordered_northings, 
                               int start_choice)
{
  int n_points = int(eastings.size());
  ...
  vector<float> new_eastings;
  vector<float> new_northings;

  list<int> remaining_points;
  list<int>::iterator it;

  int other_index;
  float test_easting,test_northing, other_easting,other_northing;
  float min_distance;
  int min_index;
  float dist_between_points;

  // A bit of logic to find the maximum and minimum easting and northing
  for (int i = 0; i< n_points; i++)
  {
    ...
    if (northings[i] > max_n)
    {
      max_n = northings[i];
      max_n_index = i;
    }
    if (northings[i] < min_n)
    {
      min_n = northings[i];
      min_n_index = i;
    }
  ...
  // now from that point we find the closest neighbour from all the other nodes, 
  // and work our way though the list of nodes, removing nodes as we go
  int test_point = 0;
  if (start_choice == 0)
  {
    test_point = max_n_index;
  }
  else if (start_choice == 1)
  {
    test_point = max_e_index;
  }
  else if (start_choice == 2)
  {
    test_point = min_n_index;
  }
  else
  {
    test_point = min_e_index;
  }

  // add the starting point to the eastings and northings
  new_eastings.push_back( eastings[test_point]);
  new_northings.push_back( northings[test_point]);

  // remove the starting point from the remaining points list. 
  remaining_points.remove(test_point);

  // now begin a while loop that keeps going until all points are removed. 
  while ( remaining_points.size() > 0)
  {
    // Reset the minimum index and distance
    min_distance = 10e9;
    min_index = 0;
    
    // get the easting and northing of the current point
    test_easting = eastings[test_point];
    test_northing = northings[test_point];

    // loop through all remaining points, to find the closest remaining point to the test point. 
    for(it = remaining_points.begin(); it != remaining_points.end(); it++)
    {
      
      other_index = *it;
      other_easting = eastings[other_index];
      other_northing = northings[other_index];

      dist_between_points = distance_between_two_points(test_easting, test_northing, other_easting, other_northing);

      if (dist_between_points < min_distance)
      {
        min_distance = dist_between_points;
        min_index = *it;
      }
    }

    // the minimum distance point is now the test point
    test_point = min_index;

    // add to the eastings and northings vector
    new_eastings.push_back( eastings[test_point]);
    new_northings.push_back( northings[test_point]);   

    // that point gets removed from the list
    remaining_points.remove(test_point);

    // and now we are ready to go back and do the whole thing again
  }

  // Okay, it is possible that there is a break in the line. Suppose your line wiggles a bit 
  // and you used max northing, so the start point is in the middle of the line. 
  // The divide would follow the path in one direction, and then skip to the
  // point not used nearest the start point. It would then continue from there. 
  // That means that from a long list of nodes, the start point would be in the middle. 
  // To fix this, we move through the nodes until we find a break, then reverse the order up to the break
  int n_nodes = new_eastings.size();

  test_easting = new_eastings[0];
  test_northing = new_northings[0];

  float first_dist = distance_between_two_points(new_eastings[0], new_northings[0], new_eastings[0], new_northings[0]);

  int node = 1;
  int jump_node = 0;
  while (node < n_nodes)
  {
    other_easting = new_eastings[node];
    other_northing = new_northings[node];
    dist_between_points = distance_between_two_points(test_easting, test_northing, other_easting, other_northing);

    if (dist_between_points > 2*first_dist) 
    {
      cout << "I found a jump! Reversing the vector!" << endl;
      jump_node = node;
      node = n_nodes;
    } 
    else
    {
      test_easting = new_eastings[node];
      test_northing = new_northings[node];
      node++;
    }    
  }

  // now reverse the vectors up to the jump vector
  reverse(new_eastings.begin(),new_eastings.begin()+jump_node);
  reverse(new_northings.begin(),new_northings.begin()+jump_node);

  // Once we have finished with all the points, replace the two vectors
  ordered_eastings = new_eastings;
  ordered_northings = new_northings;
}



void order_unstructured_points(vector<double> eastings, vector<double> northings, 
                               vector<double>& ordered_eastings, vector<double>& ordered_northings, 
                               int start_choice)
{
  int n_points = int(eastings.size());
  ...
  vector<double> new_eastings;
  vector<double> new_northings;

  list<int> remaining_points;
  list<int>::iterator it;

  int other_index;
  double test_easting,test_northing, other_easting,other_northing;
  double min_distance;
  int min_index;
  ...
  double first_dist = distance_between_two_points(new_eastings[0], new_northings[0], new_eastings[0], new_northings[0]);
...
// This function takes two pairs of easting and northing vectors
// for each point in the first pair, it calculates the minimum distance
// to the other set of points, and records this as a distance
// returns a vector of these distances, that is the same size as the first pair of vectors. 
vector<float> calculate_nearest_points_to_a_line(vector<float> eastings, vector<float> northings, 
                                                vector<float> compare_eastings, vector<float> compare_northings)
{

  int n_nodes = int(eastings.size());
  int n_compare = int(compare_eastings.size());

  float min_dist;
  float this_dist;

  vector<float> minimum_dists;
  ...
  for(int i = 0; i< n_nodes;n_nodes, i++)
  {
    ...
    {
      this_dist = distance_between_two_points(eastings[i], northings[i], compare_eastings[j], compare_northings[j]);
    ...
    minimum_dists.push_back(min_dist);
  }
